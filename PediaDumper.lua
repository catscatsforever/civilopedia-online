...
for era in GameInfo.Eras() do
	...
	for tech in GameInfo.Technologies("Era = '" .. era.Type .. "'") do
		...
		for row in GameInfo.Route_TechMovementChanges( condition ) do
			tblAbilities[#tblAbilities + 1] = Locale.ConvertTextKey("TXT_KEY_CIVILOPEDIA_SPECIALABILITIES_MOVEMENT", GameInfo.Routes[row.RouteType].Description);
		end
		for row in GameInfo.Improvement_TechYieldChanges( condition ) do
			tblAbilities[#tblAbilities + 1] = Locale.ConvertTextKey("TXT_KEY_CIVILOPEDIA_SPECIALABILITIES_YIELDCHANGES", GameInfo.Improvements[row.ImprovementType].Description, GameInfo.Yields[row.YieldType].Description, row.Yield);
		end
		for row in GameInfo.Improvement_TechNoFreshWaterYieldChanges( condition ) do
			tblAbilities[#tblAbilities + 1] = Locale.ConvertTextKey("TXT_KEY_CIVILOPEDIA_SPECIALABILITIES_NOFRESHWATERYIELDCHANGES", GameInfo.Improvements[row.ImprovementType].Description, GameInfo.Yields[row.YieldType].Description, row.Yield );
		end
		for row in GameInfo.Improvement_TechFreshWaterYieldChanges( condition ) do
			tblAbilities[#tblAbilities + 1] = Locale.ConvertTextKey("TXT_KEY_CIVILOPEDIA_SPECIALABILITIES_FRESHWATERYIELDCHANGES", GameInfo.Improvements[row.ImprovementType].Description, GameInfo.Yields[row.YieldType].Description, row.Yield );
		end
		if tech.EmbarkedMoveChange > 0 then
			tblAbilities[#tblAbilities + 1] = Locale.ConvertTextKey( "TXT_KEY_ABLTY_FAST_EMBARK_STRING" );
		end
		if tech.AllowsEmbarking then
			tblAbilities[#tblAbilities + 1] = Locale.ConvertTextKey( "TXT_KEY_ALLOWS_EMBARKING" );
		end
		if tech.AllowsDefensiveEmbarking then
			tblAbilities[#tblAbilities + 1] = Locale.ConvertTextKey( "TXT_KEY_ABLTY_DEFENSIVE_EMBARK_STRING" );
		end
		if tech.EmbarkedAllWaterPassage then
			tblAbilities[#tblAbilities + 1] = Locale.ConvertTextKey( "TXT_KEY_ABLTY_OCEAN_EMBARK_STRING" );
		end
		if tech.OpenBordersTradingAllowed then
			tblAbilities[#tblAbilities + 1] = Locale.ConvertTextKey( "TXT_KEY_ABLTY_OPEN_BORDER_STRING" );
		end
		if tech.DefensivePactTradingAllowed then
			tblAbilities[#tblAbilities + 1] = Locale.ConvertTextKey( "TXT_KEY_ABLTY_D_PACT_STRING" );
		end
		if tech.ResearchAgreementTradingAllowed then
			tblAbilities[#tblAbilities + 1] = Locale.ConvertTextKey( "TXT_KEY_ABLTY_R_PACT_STRING" );
		end
		if tech.TradeAgreementTradingAllowed then
			tblAbilities[#tblAbilities + 1] = Locale.ConvertTextKey( "TXT_KEY_ABLTY_T_PACT_STRING" );
		end
		if tech.BridgeBuilding then
			tblAbilities[#tblAbilities + 1] = Locale.ConvertTextKey( "TXT_KEY_ABLTY_BRIDGE_STRING" );
		end
